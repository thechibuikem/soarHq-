...
const Hamburger = ()=>{
  ...
  return(
<>
            <button onClick={()=>setSeeSideNav(true)} className="md:hidden cursor-pointer">
          <svg
        ...
        {/* the right hamburger */}
    <div className={`h-screen w-screen backdrop-blur-md bg-white/10 border border-white/20 shadow-lg
   flex flex-col top-[0] fixed px-[2rem] py-[1rem] z-50 ring-1 ring-white/10 rounded-xl ${seeSideNav?'flex':'hidden'}`}>
{/* the button */}
        <button onClick={()=>setSeeSideNav(false)} className="fixed right-[4rem] cursor-pointer">
            {/* element 1 */}
            <svg
            xmlns="http://www.w3.org/2000/svg"
            width="32"
